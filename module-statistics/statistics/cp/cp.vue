        ...
        <view class="w-list-wrap" style="margin-top: 54px">
            ...
            >
                        ...
                        <view class="cp-manager-name">
                            ...
                            <u-icon
                                ...
                                @click="
                                    makePhoneCall(
                                        item.ownerManager
                                            ? item.ownerManager.person.mobile
                                            : ''
                                    )
                                "
                            ></u-icon>
                ...
                <view class="cp-body">
                    <view
                        class="cp-body-label"
                        @click="toSafetyCheckList(item.id, '', '')"
                    >
                    ...
                    <view
                        class="cp-body-item"
                        @click="toSafetyCheckList(item.id, 'safe', '')"
                    >
                    ...
                    <view class="cp-body-item" style="width: 50%">
                        ...
                        <view class="item-container">
                            <view
                                class="item-content"
                                @click="
                                    toSafetyCheckList(item.id, 'risk', true)
                                "
                            >
                                <view class="success">
                            ...
                            <view
                                class="item-content"
                                @click="
                                    toSafetyCheckList(item.id, 'risk', false)
                                "
                            >
                                <view class="danger">
                ...
                <view v-if="hasAuthority" class="cp-body">
                    <view
                        class="cp-body-label"
                        @click="toSafetyTrainingList(item.id)"
                    >
                ...
                <view v-if="hasAuthority" class="cp-body">
                    ...
                    <view
                        class="cp-body-item"
                        @click="toContractList(item.id, 'not_uploaded')"
                    >
                    ...
                    <view
                        class="cp-body-item"
                        @click="toContractList(item.id, 'confirmed')"
                    >
                    ...
                    <view
                        class="cp-body-item"
                        @click="toContractList(item.id, 'unconfirmed')"
                    >
                ...
                <view v-if="hasAuthority && visible" class="cp-body">
                    <view
                        class="cp-body-label"
                        @click="toPayrollRecordList(item.id, '')"
                    >
                    ...
                    <view class="cp-body-item">
                        ...
                        <view class="item-container">
                            <view
                                class="item-content"
                                @click="toPayrollRecordList(item.id, 'paid')"
                            >
                                <view class="success">
                                    {{ item.salary.paid }}
                                </view>
                    ...
                    <view class="cp-body-item">
                        ...
                        <view class="item-container">
                            <view
                                class="item-content"
                                @click="toPayrollRecordList(item.id, 'unpaid')"
                            >
                                <view class="warning">
                                    {{ item.salary.unpaid }}
                                </view>
                    ...
                    <view class="cp-body-item">
                        ...
                        <view class="item-container">
                            <view
                                class="item-content"
                                @click="toPayrollRecordList(item.id, 'arrears')"
                            >
                                <view class="danger">
                                    {{ item.salary.arrears }}
                                </view>
    ...
    data() {
        return {
            tabs: [
                {
                    name: '日',
                    startTime: uni.atStartOfToday(),
                    endTime: uni.atStartOfTomorrow(),
                    active: false
                },
                {
                    name: '月',
    ...
    methods: {
        ...
        getList() {
            uni.request({
                ...
                success: (res) => {
                    this.listData = res.data.map((project) =>
                    ...
                    if (this.listData && this.listData.length > 0) {
                        this.statisticSafetyCheck(
                            this.listData,
                            tab.startTime,
                            tab.endTime
                        );
                        // this.statisticQualityCheck(this.listData, tab.startTime, tab.endTime);
                        if (this.hasAuthority) {
                            this.statisticSafetyTraining(
                                this.listData,
                                tab.startTime,
                                tab.endTime
                            );
                            this.statisticContract(this.listData);
                            if (this.visible) {
                                this.statisticSalary(
                                    this.listData,
                                    tab.startTime,
                                    tab.endTime
                                );
                    ...
                    this.listData.forEach((project) =>
                        res.data.forEach((data) => {
                            if (project.id === data.project) {
                                ...
                                if (data.status === 'risk') {
                                    if (data.handled) {
                                        project.safetyCheck.risk.handled =
                                            data.count;
                                    } else {
                                        project.safetyCheck.risk.unhandled =
                                            data.count;
                                    }
                                }
                            }
                    ...
                    this.listData.forEach((project) =>
                        res.data.forEach((data) => {
                            if (project.id === data.project) {
                                ...
                                if (data.status === 'risk') {
                                    if (data.handled) {
                                        project.qualityCheck.risk.handled =
                                            data.count;
                                    } else {
                                        project.qualityCheck.risk.unhandled =
                                            data.count;
                                    }
                                }
                            }
            ...
            uni.request({
                ...
                success: (res) => {
                    this.listData.forEach((project) =>
                        res.data.forEach((data) => {
                            if (project.id === data.project) {
                                project.safetyTraining[data.status] =
                                    data.count;
                            }
                        })
                    );
